...
from .types import frozendict, IdentifierIssuer, Mapping
from .const import (
    ...
    RDF_LIST, RDF_FIRST, RDF_REST, RDF_NIL, RDF_TYPE, RDF_LANGSTRING, RDF_JSON_LITERAL,
)
from .parse import (
    REGEX_BCP47, KEYWORD, ABSOLUTE_IRI,
    parse_url, unparse_url, parse_nquads, to_nquads
)
from .normalization import URDNA2015, URGNA2012
...
__all__ = [
    ...
    'load_document', 'sync_document_loader', 'async_document_loader',
...
class JsonLdProcessor:
    ...
    def expand(self, input_, options):
        ...
        :param options: the options to use.
          ...
          [extractAllScripts] `True` to extract all JSON-LD script
            elements from HTML, `False` to extract just the first.
          [processingMode] Either 'json-ld-1.0' or 'json-ld-1.1',
            defaults to 'json-ld-1.1'.
          [documentLoader(url, options)] the document loader.
        ...
        expanded = self._expand(
            active_ctx, None, document, options, inside_list=False)
    ...
    def flatten(self, input_, ctx, options):
        ...
        try:
            ...
            raise FlattenError('Could not expand input before flattening.', cause=e)
        ...
        try:
            ...
            raise FlattenError('Could not compact flattened output.', cause=e)
    ...
    def frame(self, input_, frame, options):
        ...
        try:
            ...
            raise FrameError('Could not process context before framing.', cause=e)
        ...
        try:
            ...
            raise FrameError('Could not expand input before framing.', cause=e)
        ...
        try:
            ...
            raise FrameError('Could not expand frame before framing.', cause=e)
        ...
        try:
            ...
            raise FrameError('Could not compact framed output.', cause=e)
    ...
    def normalize(self, input_, options):
        ...
        if not options['algorithm'] in ['URDNA2015', 'URGNA2012']:
            raise NormalizeError('Unsupported normalization algorithm.')

        try:
            if 'inputFormat' in options:
                if (options['inputFormat'] != 'application/n-quads' and
                        options['inputFormat'] != 'application/nquads'):
                    raise NormalizeError('Unknown normalization input format.')
                dataset = parse_nquads(input_)
            else:
    ...
    def from_rdf(self, dataset, options):
        ...
        if 'format' in options:
            ...
            if ((self.rdf_parsers is not None and
                ...
                raise UnknownFormat('Unknown input format.', format=options['format'])
    ...
    def to_rdf(self, input_, options):
        ...
        try:
            ...
            raise RdfError('Could not expand input before serialization to RDF.',
                           cause=e)
        ...
        if 'format' in options:
            if options['format'] in {'application/n-quads', 'application/nquads'}:
                return to_nquads(dataset)
            raise UnknownFormat('Unknown output format.', format=options['format'])
        return dataset
    ...
    def has_property(subject, prop):
        ...
        :return: `True` if the subject has the given property.
        """
        if prop in subject:
            value = subject[prop]
    ...
    @staticmethod
    def has_value(subject, prop, value):
        """
        ...
        """
        if JsonLdProcessor.has_property(subject, prop):
            val = subject[prop]
    ...
    def get_values(subject, prop):
        ...
        """
        return JsonLdProcessor.arrayify(subject.get(prop) or [])

    @staticmethod
    def remove_property(subject, prop):
        """
    ...
    @staticmethod
    def remove_value(subject, prop, value, options={}):
        """
        Removes a value from a subject.

        :param subject: the subject.
        :param prop: the property that relates the value to the subject.
        ...
        values = JsonLdProcessor.get_values(subject, prop)
        values = list(filter(filter_value, values))

        if len(values) == 0:
            JsonLdProcessor.remove_property(subject, prop)
        elif len(values) == 1 and not options['propertyIsArray']:
            subject[prop] = values[0]
        else:
            subject[prop] = values
    ...
    def _compact(self, active_ctx, active_property, element, options):
        ...
        if _is_object(element):
            ...
            # find all type-scoped contexts based on current context
            ...
            for expanded_property, expanded_value in sorted(element.items()):
                ...
                for expanded_item in expanded_value:
                    ...
                    # if item_active_property is a @nest property
                    #   add values to nestResult, otherwise rval
                    nest_result = rval
                    nest_property = active_ctx['mappings'].get(
                        item_active_property, {}).get('@nest')
                    if nest_property:
                    ...
                    if is_graph:
                        ...
                        if ('@graph' in container and
                                ('@id' in container or '@index' in container
                            ...
                            if '@id' in expanded_item:
                                key = self._compact_iri(active_ctx, '@id')
                                compacted_item[key] = expanded_item['@id']

                            # include @index from expanded graph, if any
                            if '@index' in expanded_item:
                                key = self._compact_iri(active_ctx, '@index')
                        ...
                        if '@language' in container:
                            ...
                            if index_key == '@index':
                                ...
                                if not indexes or not _is_string(indexes[0]):
                                    ...
                                    key = indexes.pop(0)
                                if _is_string(key):
                                    if (index_key and len(indexes) == 0
    ...
    def _expand(
        ...
        if '@value' in rval:
            ...
            if (self._processing_mode(active_ctx, 1.1)
                    and '@json' in types and len(types) == 1):
            ...
            if expanded_property == '@type':
                if _is_object(value):
                    ...
                    for k, v in value.items():
                        key = self._expand_iri(type_scoped_ctx, k, vocab=True)
                        new_value[key] = [self._expand_iri(
                                              type_scoped_ctx, vv, vocab=True,
                                              base=options.get('base', ''))
                ...
                for v in JsonLdProcessor.arrayify(value):
                    expanded_values.append(
                        self._expand_iri(
                            type_scoped_ctx, v, vocab=True,
                            base=options.get('base', '')) if _is_string(v) else v)
                JsonLdProcessor.add_value(
                    expanded_parent, '@type', expanded_values,
                    {'propertyIsArray': options['isFrame']})
                continue

            # Included blocks are treated as an array of separate object nodes
            #   sharing the same referencing active_property.
    ...
    def _process_context(
        ...
        if (_is_object(resolved[0].document)
                and isinstance(resolved[0].document.get('@propagate'), bool)):
        ...
        for resolved_context in resolved:
            ...
            if '@import' in ctx:
                ...
                if isinstance(processed_import, dict):
                    ...
                    resolved_import.set_processed(active_ctx, frozendict(ctx))
        ...
        for id_, node in sorted(graph.items()):
            for property, items in sorted(node.items()):
                ...
                for item in items:
                    ...
                    obj = self._object_to_rdf(item, issuer, triples, options.get('rdfDirection'))
    ...
    def _rdf_to_object(self, o, use_native_types, rdf_direction):
        ...
        # add datatype
            ...
            if use_native_types:
                ...
                if language:
                    rval['@language'] = language
                    if not REGEX_BCP47.match(language):
    ...
    def _add_frame_output(self, parent, prop, output):
        """
        ...
        if _is_object(parent):
            JsonLdProcessor.add_value(
                parent, prop, output, {'propertyIsArray': True})
        else:
                ...
                if _is_value(value):
                    if '@language' in value and '@index' not in value:
                        ...
                        if '@direction' in value:
                            type_or_language_value = '{@language}_{@direction}'.format(**value)
                        else:
                            type_or_language_value = value['@language']
                    elif '@direction' in value and '@index' not in value:
                            type_or_language_value = f'_{value["@direction"]}'
                    elif '@type' in value:
                        type_or_language = '@type'
    ...
    def _get_initial_context(self, options):
        ...
        if pm not in INITIAL_CONTEXTS:
            INITIAL_CONTEXTS[pm] = frozendict({'_uuid': str(uuid.uuid1()), 'processingMode': pm, 'mappings': {}})
...
def _is_object(v):
    ...
    return isinstance(v, Mapping)  # or isinstance(v, frozendict)
...
def _is_subject(v):
    ...
    if _is_object(v) and not {'@value', '@set', '@list'} & v.keys():
        return len(v) > 1 or '@id' not in v
    return False
...
def _is_graph(v):
    ...
    return (_is_object(v) and '@graph' in v and
        len([k for k in v if k != '@id' and k != '@index']) == 1)
...
def _is_absolute_iri(v):
    ...
    """
    return _is_string(v) and ABSOLUTE_IRI.match(v)
...
def _is_relative_iri(v):
    ...
    :return: True if the value is an absolute IRI.
    """
    # FIXME
    return _is_string(v)
...
def load_document(url,
    ...
    if remote_doc['document'] is None:
        raise NullRemoteDocument(code='loading document failed')
...
register_rdf_parser('application/n-quads', parse_nquads)
register_rdf_parser('application/nquads', parse_nquads)
